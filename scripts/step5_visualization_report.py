...
STEP5_EPOCH_PATH = ROOT / "data_clean" / "step5" / "02_step5_epoch_manifest.csv"
...
FIGURES_DIR = ROOT / "figures" / "step5_visualization"
REPORTS_DIR = ROOT / "reports" / "step5_visualization"
...
def load_inputs() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    epoch_df = pd.read_csv(STEP5_EPOCH_PATH)
    summary_df = pd.read_csv(STEP5_SUMMARY_PATH)
    status_df = pd.read_csv(STEP3_STATUS_PATH)
    return epoch_df, summary_df, status_df


def get_representative_session(summary_df: pd.DataFrame) -> tuple[str, str]:
    candidate_columns = ["included_step5_analysis", "included_primary_analysis"]
    included_df = summary_df.copy()
    for column in candidate_columns:
        if column in included_df.columns:
            included_df = included_df[included_df[column]].copy()
            break
...
def plot_sensor_geometry(raw: mne.io.BaseRaw, participant_id: str, session_id: str) -> None:
    ...
    fig.suptitle(
        f"Representative Step 5 sensor geometry: {participant_id} / {session_id}\n"
...
def build_visualization_dataset(epoch_df: pd.DataFrame, status_df: pd.DataFrame) -> dict[str, Any]:
    included_df = epoch_df[epoch_df["included_step5_trial"]].copy()
    ...
    if template_info is None or times is None:
        raise RuntimeError("No valid Step 5 sessions were available to build visualization plots.")
...
def plot_trial_consistency(condition: str, condition_epochs: np.ndarray, times: np.ndarray, path: Path) -> None:
    ...
    fig.suptitle(
        f"Step 5 response consistency across trials: {condition}\n"
        f"Condition-adapted version of the example plot_image(combine='mean') view"
    )
...
def plot_channel_consistency(grand_evokeds: dict[str, mne.Evoked], times: np.ndarray, path: Path) -> None:
    ...
    fig.suptitle(
        "Step 5 response consistency across channels\n"
...
def write_report(representative_session: tuple[str, str], metadata: dict[str, Any]) -> Path:
    overview_rows = [
        ("Representative geometry session", f"{representative_session[0]} / {representative_session[1]}"),
        ("Included participant-sessions from Step 5", metadata["n_included_sessions"]),
    ...
    lines = [
        ...
        r"\usepackage{float}",
        r"\begin{document}",
        r"\section*{Supplementary Step 5 Visualization Report}",
        r"\subsection*{Overview}",
        make_latex_table(overview_rows),
        r"\paragraph{Adaptation note.} "
        r"The reference examples were written for a motor task with Tapping, Control, Left, and Right conditions. "
        r"For the present experiment, those plot families were adapted to the two available task conditions, Abstract and Concrete. "
        r"All descriptive response-consistency and topographic plots in this report use the Step~5 long-channel montage so that the visualization set stays aligned with the fixed-window ROI analysis.",
        r"\paragraph{Rendering note.} "
        r"As in the Step~4 supplementary visualization report, the sensor-geometry figure uses the participant digitization point cloud plus source, detector, channel, and pair geometry rather than an fsaverage brain-surface rendering, because the local \texttt{pyvista}/\texttt{fsaverage} stack is not available in this environment.",
        r"\subsection*{Step 5 Main Figures Reused}",
        r"\begin{figure}[H]",
        r"\centering",
        ...
        r"\caption{Participant-level fixed-window HbO summaries for the front and back ROIs from the main Step~5 report.}",
        r"\end{figure}",
        r"\begin{figure}[H]",
        r"\centering",
        ...
        r"\caption{Step~5 onset-locked HbO waveforms with the primary 7--11~s analysis window shaded.}",
        r"\end{figure}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.48\linewidth]{../../figures/step5/dissociation_histogram_fixed_window.png}",
        r"\includegraphics[width=0.48\linewidth]{../../figures/step5/topography_9s.png}",
        r"\caption{Fixed-window dissociation histogram and the Step~5 single-time HbO topography at 9.0~s.}",
        r"\end{figure}",
        r"\subsection*{Sensor Geometry}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.95\linewidth]{../../figures/step5_visualization/representative_sensor_geometry.png}",
        r"\caption{Representative optode, channel, and source-detector-pair geometry for one Step~5 included participant-session. Short pairs are shown with dashed orange lines.}",
        r"\end{figure}",
        r"\subsection*{Consistency Across Trials}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.48\linewidth]{../../figures/step5_visualization/abstract_trial_consistency.png}",
        r"\includegraphics[width=0.48\linewidth]{../../figures/step5_visualization/concrete_trial_consistency.png}",
        r"\caption{Condition-adapted versions of the trial-consistency image plots. Each panel shows the mean long-channel HbO and HbR response across individual trials for Abstract and Concrete questions separately.}",
        r"\end{figure}",
        r"\subsection*{Consistency Across Channels}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.95\linewidth]{../../figures/step5_visualization/channel_consistency_grid.png}",
        r"\caption{Grand-average long-channel response images across channels for the Abstract and Concrete conditions. Rows correspond to HbO and HbR, and columns correspond to the two conditions.}",
        r"\end{figure}",
        r"\subsection*{Standard fNIRS Waveform Summary}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.92\linewidth]{../../figures/step5_visualization/standard_fnirs_compare.png}",
        r"\caption{Condition-adapted standard fNIRS waveform plot. Solid lines indicate Abstract, dashed lines indicate Concrete, magenta shows HbO, and blue shows HbR. Shaded bands show 95\% confidence intervals across participant-sessions.}",
        r"\end{figure}",
        r"\subsection*{Topographic Representation Over Time}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.95\linewidth]{../../figures/step5_visualization/abstract_hbo_joint.png}",
        r"\caption{Grand-average HbO topography over time for Abstract questions using the Step~5 included trial set.}",
        r"\end{figure}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.95\linewidth]{../../figures/step5_visualization/concrete_hbo_joint.png}",
        r"\caption{Grand-average HbO topography over time for Concrete questions using the Step~5 included trial set.}",
        r"\end{figure}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.95\linewidth]{../../figures/step5_visualization/abstract_minus_concrete_hbo_joint.png}",
        r"\caption{Grand-average Abstract-minus-Concrete HbO topography over time.}",
        r"\end{figure}",
        r"\subsection*{Condition Topomap Series}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.48\linewidth]{../../figures/step5_visualization/abstract_hbo_topomap_series.png}",
        r"\includegraphics[width=0.48\linewidth]{../../figures/step5_visualization/concrete_hbo_topomap_series.png}",
        r"\caption{HbO topographic time series from 4.0~s to 10.0~s for the Abstract and Concrete conditions.}",
        r"\end{figure}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.48\linewidth]{../../figures/step5_visualization/abstract_hbr_topomap_series.png}",
        r"\includegraphics[width=0.48\linewidth]{../../figures/step5_visualization/concrete_hbr_topomap_series.png}",
        r"\caption{HbR topographic time series from 4.0~s to 10.0~s for the Abstract and Concrete conditions.}",
        r"\end{figure}",
        r"\subsection*{Single-Time Condition Comparison}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.95\linewidth]{../../figures/step5_visualization/abstract_concrete_single_time_comparison.png}",
        ...
        r"\end{figure}",
        r"\subsection*{Channel-Wise Waveform Topography}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.95\linewidth]{../../figures/step5_visualization/abstract_concrete_hbo_evoked_topo.png}",
        r"\caption{HbO waveforms by long channel, overlaid for Abstract and Concrete, to show which local waveforms drive the descriptive topographic differences.}",
        r"\end{figure}",
        r"\end{document}",
    ]

    report_path = REPORTS_DIR / "step5_visualization_report.tex"
...
def main() -> None:
    ensure_directories()
    epoch_df, summary_df, status_df = load_inputs()
    ...
    dataset = build_visualization_dataset(epoch_df, status_df)
    times = dataset["times"]
    ...
    print("Step 5 visualization report generated successfully.")
    ...
    print(f"Long pairs used in descriptive plots: {metadata['n_long_pairs']}")
